...
Analyst `hold` 不授权 retry。`reassess` 只创建新的可审计 Incident/Analysis cycle，也不直接授权执行。same-HEAD、settled、无 approval 的 Reviewer `blocked` 或 `failed` durable result 都可在基础设施修复后进入该重评路径；两者仍必须由新 Analysis 与精确 human gate 才能创建 fresh Reviewer。
...
| `controller/runtime-preflight.ts` | Provider、Docker、Pi 与 execution snapshot gate，以及不推进 workflow 的 bounded preflight report |
