...
test("durable runner disables retry and compaction before dispatch and settles through result", () => {
  ...
  try {
    ...
    assert.equal(readJson<{ ambientAuthExcluded: boolean }>(join(plan.runtimeRoot, "ready.json")).ambientAuthExcluded, true);
    ...
    assert.deepEqual(readJson<Record<string, { enabled: boolean }>>(join(isolatedAgentDir, "settings.json")), {
      retry: { enabled: false },
...
test("durable runner fails closed when RPC JSONL breaks before or after settlement", () => {
  for (const malformedVariable of ["FAKE_PI_MALFORMED_AFTER_PROMPT", "FAKE_PI_MALFORMED_AFTER_SETTLED"] as const) {
    const fixture = rpcFixture();
    const previous = {
      result: process.env.FAKE_PI_RESULT_PATH,
      job: process.env.FAKE_PI_JOB_ID,
