...
export function controllerHeartbeatPath(stateDir: string): string {
  if (!isAbsolute(stateDir)) throw new Error("heartbeat stateDir must be absolute");
  return join(stateDir, "controller-heartbeat.json");
}

export function startControllerHeartbeat(stateDir: string, intervalMs = DEFAULT_INTERVAL_MS): { stop(): void } {
  ...
  if (!child.pid) throw new Error("Controller heartbeat process did not start");
  child.unref();
