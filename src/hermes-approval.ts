...
async function main(argv: string[]): Promise<number> {
  ...
  if (command === "request") return requestChallenge(config);
  return confirmChallenge(config);
}

async function requestChallenge(config: ApprovalConfig): Promise<number> {
...
async function confirmChallenge(config: ApprovalConfig): Promise<number> {
