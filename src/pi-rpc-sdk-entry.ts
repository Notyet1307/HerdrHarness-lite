...
type ModelRuntime = {
  ...
  getAuth(model: Model, options: Record<string, unknown>): Promise<unknown>;
...
async function main(argv: string[]): Promise<void> {
  ...
  if (oauthAgentDir === privateAgentDir) throw new Error("Pi RPC OAuth and private agent directories must differ");
  ...
  const http = await import(pathToFileURL(join(dirname(piIndex), "core", "http-dispatcher.js")).href) as {
