...
function renderWhy(state: HarnessState): string {
  ...
  if (isFailedClosedAnalysis(analysis)) return renderFailedClosedWhy(job);
...
function isFailedClosedAnalysis(analysis: AnalystAdvice): boolean {
  return analysis.summary.startsWith("Analyst diagnosis failed closed:");
}

function renderFailedClosedWhy(job: Job): string {
  ...
  const blockingFindings = reviewerResult?.findings.filter((finding) => (
  ...
  if (displayedFindings.length > 0) {
    lines.push(
      "",
      "关键依据",
      ...displayedFindings.map((finding) => `• ${finding.severity} · ${clean(finding.summary, 230)}`),
      ...(reviewerResult && reviewerResult.findings.length > displayedFindings.length
