...
test("Hermes Fleet aggregates status and routes cards or callbacks to one fixed lane", () => {
    ...
    try {
        ...
        for (const [path, laneId] of [[exposure, "exposure"], [tailmux, "tailmux"]]) {
...
function invoke(config, args, allowedUser, allowAll = "") {
...
function invokeFleet(config, args, allowedUser) {
    return spawnSync("python3", ["-c", PYTHON, resolve("integrations/hermes-telegram/plugin/__init__.py")], {
        encoding: "utf8",
        timeout: 5_000,
...
function fleetEnv(config, extra) {
    return {
        ...process.env,
        PYTHONDONTWRITEBYTECODE: "1",
...
//# sourceMappingURL=hermes-plugin.test.js.map