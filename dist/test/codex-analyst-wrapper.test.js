...
test("Codex Analyst start binds one real session to one job and task digest", () => {
    ...
    try {
        ...
        const payloadDrift = startRequest("a".repeat(64));
...
test("Codex Analyst turn is replay-safe and bound to the evidence digest", () => {
    ...
    try {
        ...
        const contentDrifted = turnRequest(session, "c".repeat(64));
        ...
        assert.match(calls[1].at(-1), /hypotheses must contain between 1 and 5 entries/);
...
function createCrashingFixture() {
...
function callWrapper(fixture, request) {
    const result = spawnSync(process.argv[0], ["dist/src/codex-analyst-wrapper.js", "--state-dir", fixture.stateDir, "--codex-bin", fixture.codexBin], { encoding: "utf8", input: `${JSON.stringify(request)}\n`, timeout: 5_000 });
    return { status: result.status, stdout: result.stdout, stderr: result.stderr };
}
function startRequest(taskDigest) {
    return {
        operation: "start",
        jobId: "job-1",
        task: {
...
function turnRequest(started, evidenceDigest) {
    ...
    return {
        ...
        job: {
            ...
            revision: 7,
            state: "blocked",
            task: { ...(startRequest(taskDigest).task) },
...
//# sourceMappingURL=codex-analyst-wrapper.test.js.map