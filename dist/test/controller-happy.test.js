...
import { FakeAnalyst, FakeClock, FakeEvidence, FakeGit, FakeGitHub, FakeHerdr, FakeRuntimePreflight, MemoryStore, SequenceIds, issue, substituteCodeReviewSkillPath, substituteTddSkillPath, untrustedImplementSkillPath, validCodeReviewSkillPath, validImplementSkillPath, validPiSubagentsExtensionPath, validReviewerArgv, validWorkerArgv, } from "./fakes.js";
const config = {
    repo: "owner/repo",
    localPath: "/repo",
    stateDir: "/state",
...
test("config rejects incomplete Pi role contracts", () => {
    for (const invalidConfig of [
        ...
        {
            ...config,
            reviewerArgv: validReviewerArgv.map((value) => (value === "read,grep,find,ls,subagent,review_preflight,review_validate,review_submit" ? `${value},write` : value)),
        },
    ]) {
        assert.throws(() => new HarnessController({
            config: invalidConfig,
...
test("config rejects an unqualified pi-subagents version", () => {
    ...
    try {
        ...
        const reviewerArgv = validReviewerArgv.map((value) => (value === validPiSubagentsExtensionPath ? join(packageRoot, "index.js") : value));
        assert.throws(() => new HarnessController({
...
test("Attempt binds one immutable execution snapshot and ignores later config drift", async () => {
    ...
    const runtimeConfig = { ...config, workerArgv: [...validWorkerArgv] };
    const preflight = new FakeRuntimePreflight();
    const controller = new HarnessController({
...
test("Attempt fails closed when its plan or inspected runtime drifts", async () => {
    for (const mutate of [
        (store, preflight) => { preflight.version = "0.85.0"; },
        (store) => { store.state.activeJob.activeAttempt.executionSnapshot.argv.push("--no-session"); },
        (store) => { store.state.activeJob.activeAttempt.contextEnvelope.task.objective = "tampered"; },
    ]) {
        const store = new MemoryStore();
        const preflight = new FakeRuntimePreflight();
...
test("Attempt blocks before pane creation when trusted context or ambient SYSTEM state drifts", async () => {
    for (const breakBoundary of [
        (git) => { git.trustedContextFailure = new Error("bundle digest changed"); },
        (_git, preflight) => { preflight.ambientFailure = new Error("global SYSTEM appeared"); },
    ]) {
        const store = new MemoryStore();
...
test("happy path claims, starts Analyst, runs fresh Pi worker/reviewer, publishes, and archives", async () => {
    ...
    assert.deepEqual(JSON.parse(Buffer.from(herdr.prepared[1].env.PI_SUBAGENT_CAPABILITY_CEILING_V1, "base64url").toString("utf8")), {
        version: 1,
