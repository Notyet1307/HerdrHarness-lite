...
export function operatorActionsFor(job) {
    const incident = job.incident;
    ...
    if (reassessmentClassFor(job) !== null)
        actions.push(makeOperatorAction(job, "reassess", "rerun_analysis"));
...
export function reassessmentClassFor(job) {
    const incident = job.incident;
    const analysis = job.analysis;
    if (job.state !== "blocked"
        || !incident
...
function makeOperatorAction(job, kind, effect) {
    const binding = {
        jobId: job.id,
        revision: job.revision,
...
function isLegacyWorkerHeadMismatch(job) {
    const attempt = job.activeAttempt;
    const incident = job.incident;
