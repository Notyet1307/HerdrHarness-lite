...
import { MAX_ANALYST_HYPOTHESES, MAX_ANALYST_UNKNOWNS, parseAnalystTurn } from "./adapters/json-command-analyst.js";
...
function start(options, value) {
    ...
    if (existing)
        return existingStart(existing, request, requestDigest);
    ...
    if (!result.ok || parsed.malformed || !parsed.ready || !parsed.sessionId) {
        const detail = boundedError(result.ok
            ? parsed.malformed
    ...
    const active = { ...reserved, status: "active", sessionId: parsed.sessionId };
    replace(receiptPath, active);
    return startResponse(active);
}
function existingStart(receipt, request, requestDigest) {
...
function turn(options, value) {
    ...
    const result = runner.run(options.codexBin, ["exec", "resume", ...restrictedArgs(false), request.session.id, analystTurnPrompt(request)], { cwd: analystDirectory(options.stateDir), timeoutMs: 105_000 });
...
function close(options, value) {
    const request = closeRequest(value);
    const receiptPath = startReceiptPath(options.stateDir, request.jobId);
    const receipt = readStartReceipt(receiptPath);
    if (!receipt) {
        if (request.session)
            throw new Error("refusing to close an unrecorded Codex Analyst session");
        return { status: "noop" };
    }
    ...
    if (receipt.status === "closed")
        return { status: "closed", ...(receipt.sessionId ? { sessionId: receipt.sessionId } : {}) };
    if (!receipt.sessionId)
        throw new Error("Codex Analyst start has no recorded UUID; exact cleanup is unavailable");
...
function analystTurnPrompt(request) {
    return [
        ...
        `For advice, diagnosis.hypotheses must contain between 1 and ${MAX_ANALYST_HYPOTHESES} entries. Consolidate related hypotheses instead of exceeding this limit.`,
...
function restrictedArgs(initial) {
    return [
        "--strict-config",
        "--ignore-user-config",
        "--ignore-rules",
...
function parseCodexStart(result) {
    if (result.stdout.length > 1_000_000)
        return { sessionId: null, ready: false, malformed: true };
    let sessionId = null;
    let ready = false;
    let malformed = false;
    let sessionEvents = 0;
...
function parseCodexTurn(result, expectedSessionId) {
    if (result.stdout.length > 1_000_000)
        return { response: null, error: "Codex Analyst JSONL exceeded the size limit" };
    let message = null;
    let sessionId = null;
    for (const line of result.stdout.split("\n")) {
        if (!line.trim())
        ...
        if (event.type === "item.completed" && record(event.item) && event.item.type === "agent_message" &&
            ...
            if (message !== null)
                return { response: null, error: "Codex Analyst returned multiple final messages" };
    ...
    try {
        return { response: parseAnalystTurn(JSON.parse(message)), error: null };
    }
    catch (error) {
...
function readRequest() {
    ...
    if (!raw.trim() || raw.length > 1_000_000 || raw.includes("\u0000"))
        throw new Error("invalid Analyst request size");
    const value = JSON.parse(raw);
    if (!record(value))
        throw new Error("Analyst request is not an object");
    return value;
}
...
function analystDirectory(stateDir) {
...
function startReceiptPath(stateDir, jobId) {
    return join(analystDirectory(stateDir), `start-${sha256(jobId)}.json`);
}
function turnReceiptPath(stateDir, request) {
    return join(analystDirectory(stateDir), `turn-${sha256(`${request.session.id}\n${request.evidence.incidentId}\n${request.turn}`)}.json`);
}
function readStartReceipt(path) {
    if (!existsSync(path))
        return null;
    const value = JSON.parse(readFileSync(path, "utf8"));
    if (!validStartReceipt(value))
        throw new Error("invalid Codex Analyst start receipt");
    return value;
}
function reserve(path, receipt, operation) {
    try {
        ...
        if (existsSync(path))
            throw new Error(`Codex Analyst ${operation} was reserved concurrently; retry without dispatching`);
        throw error;
    }
}
function readTurnReceipt(path) {
    if (!existsSync(path))
        return null;
    const value = JSON.parse(readFileSync(path, "utf8"));
    if (!validTurnReceipt(value))
        throw new Error("invalid Codex Analyst turn receipt");
    return value;
}
function replace(path, receipt) {
...
function validStartReceipt(value) {
    if (!record(value))
        return false;
...
//# sourceMappingURL=codex-analyst-wrapper.js.map