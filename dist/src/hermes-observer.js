...
function enqueueAutomaticRecoveries(observer, job, offset) {
    const recoveries = job.automaticRecoveries ?? [];
    if (recoveries.length <= offset)
        return false;
...
function enqueueCurrentAutomaticRecovery(observer, job) {
...
function automaticRecoveryNotice(job, recovery) {
...
function loadState(path) {
    ...
    const value = {
        ...migrated,
        lastAutomaticRecoveryCount: Number.isInteger(migrated.lastAutomaticRecoveryCount)
            ? migrated.lastAutomaticRecoveryCount
            : 0,
    };
    if (value.version !== 2
