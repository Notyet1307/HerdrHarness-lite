...
function renderWhy(state) {
    ...
    if (!job?.incident)
        return "当前没有可解释的 blocked incident。";
    ...
    if (diagnosis?.hypotheses.length) {
        lines.push("", "关键依据", ...diagnosis.hypotheses.slice(0, 2).map((hypothesis) => (`• ${hypothesis.status} · ${clean(hypothesis.claim, 230)}`)), ...(diagnosis.hypotheses.length > 2 ? [`• …另 ${diagnosis.hypotheses.length - 2} 项见证据索引`] : []));
    }
...
function renderEvidence(state) {
    const job = state.activeJob;
    if (!job?.incident)
        return "当前没有 blocked incident 的证据索引。";
    const analysis = job.analysis?.incidentId === job.incident.id ? job.analysis : null;
    if (!analysis)
        return "当前 incident 尚无精确绑定的 durable analysis。";
    return [
        "🔎 证据索引",
        `${mobileTask(job)} · rev ${job.revision}`,
        "",
...
function renderActions(state, laneId) {
    const projection = projectOperatorState(state);
    if (!projection.jobId)
        return "当前没有活跃任务。";
    if (projection.actions.length === 0)
        return `当前 revision ${projection.revision} 没有可执行 operator action。`;
...
function operatorActionCommand(kind) {
    if (kind === "approve_retry")
        return "retry";
    if (kind === "resolve_decision")
        return "resolve <reason>";
    if (kind === "reassess")
        return "reassess <reason>";
...
function renderStatus(state, config) {
    ...
    if (job.pullRequest)
        lines.push(`• PR #${job.pullRequest.number} ${clean(job.pullRequest.url, 220)}`);
    lines.push("", "运行信息", `• Worker：${clean(runtimeSelection(config.workerArgv), 230)}`, `• Reviewer：${clean(runtimeSelection(resolveReviewerProviderProfile(config.reviewerArgv, config.reviewerProviderProfiles).argv), 230)}`, `• 本轮：${clean(activeRuntime(job), 230)}`, `• 更新：${displayTime(job.updatedAt)}`, "", "下一步", clean(nextStep(job, projection), 260));
    return lines.join("\n");
}
...
function renderIncident(state, laneId) {
    ...
    lines.push("", "Analyst 判断", clean(presentedAnalysisSummary(analysis), 260), `建议动作：${analysis.action}`);
