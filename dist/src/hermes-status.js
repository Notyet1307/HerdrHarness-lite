...
function renderWhy(state) {
    ...
    if (diagnosis?.hypotheses.length) {
        lines.push("假设检验：", ...diagnosis.hypotheses.map((hypothesis) => (`- ${hypothesis.status}/${hypothesis.confidence}：${clean(hypothesis.claim, 420)} [${hypothesis.evidenceRefs.join(", ") || "无引用"}]`)));
    }
    ...
    if (analysis.unknowns.length)
        lines.push(`仍未知：${analysis.unknowns.map((value) => clean(value, 280)).join("；")}`);
...
function renderActions(state, laneId) {
    ...
    return [
        ...
        "精确绑定操作：",
        ...projection.actions.map((action) => (`- ${operatorActionLabel(action)}\n  命令：${prefix} ${operatorActionCommand(action.kind)}`)),
