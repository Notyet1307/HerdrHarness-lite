...
export class GitCli {
    ...
    async prepareTrustedContext(input) {
        if (!/^[0-9a-f]{40}$/i.test(input.trustAnchorSha))
            throw new Error("trusted context requires an exact commit SHA");
        const rootPath = resolve(input.rootPath);
        if (pathsOverlap(input.localPath, rootPath))
            throw new Error("trusted context state must be outside the product repository");
        const entries = [];
        let policy = "";
        for (const path of CONTEXT_CANDIDATES) {
            const tree = requireSuccess(this.runner.run("git", ["-C", input.localPath, "ls-tree", input.trustAnchorSha, "--", path]), `git read trusted context entry ${path}`).trim();
            if (!tree)
                continue;
            ...
            policy = requireSuccess(this.runner.run("git", ["-C", input.localPath, "show", `${input.trustAnchorSha}:${path}`]), `git read trusted context blob ${path}`);
            if (policy.includes("\0"))
                throw new Error(`trusted context contains NUL: ${path}`);
            if (Buffer.byteLength(policy, "utf8") > MAX_CONTEXT_BYTES)
                throw new Error(`trusted context exceeds ${MAX_CONTEXT_BYTES} bytes: ${path}`);
            entries.push({
    ...
    async verifyTrustedContext(context) {
        ...
        for (const path of [context.bundlePath, context.manifestPath]) {
            if (lstatSync(path).mode & 0o222)
                throw new Error(`trusted context artifact is writable: ${path}`);
        }
        const expected = {
    ...
    async prepareReviewer(input) {
        ...
        if (!input.piRuntimeVersion.trim() || /[\0\r\n]/.test(input.piRuntimeVersion))
            throw new Error("Reviewer Pi runtime version is invalid");
        if (!isAbsolute(input.piAgentDir) || /[\0\r\n]/.test(input.piAgentDir))
            throw new Error("Reviewer Pi agent directory is invalid");
...
function piSubagentWrapper(executable, runtimeVersion, emptyAppendSystemPromptPath) {
...
function writeImmutable(path, content) {
    if (existsSync(path)) {
        if (readFileSync(path, "utf8") !== content)
            throw new Error(`trusted context identity changed after preparation: ${path}`);
        if (lstatSync(path).mode & 0o222)
            throw new Error(`trusted context artifact is writable: ${path}`);
        return;
    }
