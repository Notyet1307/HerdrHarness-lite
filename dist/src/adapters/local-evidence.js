...
export class LocalEvidence {
    runner;
    stateDir;
    constructor(runner = new SyncCommandRunner(), stateDir = null) {
        this.runner = runner;
    ...
    async initial(job) {
        const items = [
            ...
            item("job", "ledger.job", JSON.stringify({
                id: job.id,
        ...
        if (job.activeAttempt)
            items.push(item("active-attempt", "ledger.activeAttempt", JSON.stringify(attemptSummary(job.activeAttempt))));
        const lastReview = [...job.attempts].reverse().find((attempt) => attempt.lane === "reviewer" && attempt.result);
        if (lastReview)
            items.push(item("last-review", "ledger.attempts", JSON.stringify(lastReview.result)));
        const missing = [];
        if (!job.worktree)
    ...
    collectOne(job, request) {
        switch (request.kind) {
            case "issue_context":
                return item("issue-context", "ledger.task", JSON.stringify(job.task));
            case "attempt_result":
                return item(`attempt-${job.activeAttempt?.id ?? "none"}`, "ledger.activeAttempt", JSON.stringify(job.activeAttempt?.result ?? (job.activeAttempt ? attemptSummary(job.activeAttempt) : null)));
    ...
    worktreeProgress(job) {
        ...
        for (const [name, args] of Object.entries(commands))
            output[name] = this.runGit(job.worktree.path, args);
    ...
    attemptHistory(job) {
        ...
        if (job.activeAttempt && !attempts.some((entry) => entry.id === job.activeAttempt.id))
            attempts.push(job.activeAttempt);
    ...
    runtimeDirectory(jobId, lane, attemptId) {
        if (this.stateDir === null)
            return null;
...
function receiptSummary(runtime) {
    return Object.fromEntries(["owner", "ready", "accepted", "terminal", "terminated"].map((name) => [
        name,
        readSelectedJson(join(runtime, `${name}.json`)),
    ]));
}
function readSelectedJson(path) {
    if (!existsSync(path))
        return null;
    try {
...
function item(ref, source, summary) {
...
//# sourceMappingURL=local-evidence.js.map