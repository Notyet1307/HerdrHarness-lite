...
export class LocalEvidence {
    ...
    async initial(job) {
        ...
        if (job.activeAttempt)
            items.push(item("active-attempt", "ledger.activeAttempt", JSON.stringify(job.activeAttempt)));
    ...
    collectOne(job, request) {
        switch (request.kind) {
            ...
            case "attempt_result":
                return item(`attempt-${job.activeAttempt?.id ?? "none"}`, "ledger.activeAttempt", JSON.stringify(job.activeAttempt?.result ?? job.activeAttempt ?? null));
            case "git_status": {
                if (!job.worktree)
                    return null;
