...
async function main(argv) {
    ...
    if (credentialAgentDir === privateAgentDir)
        throw new Error("Pi RPC credential and private agent directories must differ");
    const authPath = join(credentialAgentDir, "auth.json");
    const assertCredentialInputs = () => {
    ...
    const createRuntime = async (input) => {
        ...
        if (host.credentialMode === "canonical-oauth") {
            ...
            const providerConfig = loadBoundProviderConfig(host.modelConfigPath, host.modelConfigDigest, runtimeArgs.provider);
...
function assertCanonicalModelConfig(path, expectedDigest) {
    ...
    if (executionResourceDigest(path) !== expectedDigest)
        throw new Error("models.json changed after Attempt preparation");
}
function loadBoundProviderConfig(path, expectedDigest, provider) {
    ...
    const root = exactObject(JSON.parse(content), ["providers"], "root");
    ...
    for (const value of definitions) {
        ...
        if (models.some((model) => model.id === definition.id))
            throw new Error(`models.json has duplicate model ${String(definition.id)}`);
...
function normalizeModelDefinition(value, providerConfig, providerApi, providerBaseUrl, overrides) {
    ...
    const thinking = mergeRecord(definition.thinkingLevelMap === undefined ? undefined : thinkingMap(definition.thinkingLevelMap), override.thinkingLevelMap === undefined ? undefined : thinkingMap(override.thinkingLevelMap));
    const sampling = mergeRecord(definition.samplingParams === undefined ? undefined : object(definition.samplingParams), override.samplingParams === undefined ? undefined : object(override.samplingParams));
    const headers = {
    ...
    const compat = mergeCompat(mergeCompat(providerConfig.compat === undefined ? undefined : compatObject(providerConfig.compat, "provider compat"), definition.compat === undefined ? undefined : compatObject(definition.compat, `model ${id} compat`)), override.compat === undefined ? undefined : compatObject(override.compat, `model ${id} override compat`));
    return {
        id,
...
function normalizeModelOverride(value, id) {
    ...
    if (override.thinkingLevelMap !== undefined)
        thinkingMap(override.thinkingLevelMap);
    if (override.input !== undefined)
        inputArray(override.input, id);
    if (override.cost !== undefined)
        costObject(override.cost, true);
    positiveNumber(override, "contextWindow");
    positiveNumber(override, "maxTokens");
    if (override.samplingParams !== undefined)
        object(override.samplingParams);
    if (override.headers !== undefined)
        stringMap(override.headers, `model ${id} override headers`);
    if (override.compat !== undefined)
        compatObject(override.compat, `model ${id} override compat`);
    return override;
}
function modelConfigContentDigest(content) {
    const hash = createHash("sha256");
    hash.update(".\0");
    hash.update(content);
    hash.update("\0");
...
function object(value) {
    if (!value || typeof value !== "object" || Array.isArray(value))
        throw new Error("models.json has an invalid object shape");
    return value;
}
function exactObject(value, allowed, label) {
    ...
    for (const key of Object.keys(record)) {
        if (!allowed.includes(key))
            throw new Error(`models.json ${label} has unsupported field ${key}`);
    }
    return record;
}
function requiredString(record, key) {
    ...
    if (typeof value !== "string" || value.length === 0)
        throw new Error(`models.json ${key} must be a non-empty string`);
    return value;
}
...
function requiredBoolean(record, key) {
    ...
    if (typeof value !== "boolean")
        throw new Error(`models.json ${key} must be boolean`);
    return value;
}
...
function positiveNumber(record, key) {
    ...
    if (typeof value !== "number" || !Number.isFinite(value) || value <= 0)
        throw new Error(`models.json ${key} must be positive`);
    return value;
}
function stringMap(value, label) {
    ...
    for (const [key, entry] of Object.entries(record)) {
        if (typeof entry !== "string")
            throw new Error(`models.json ${label}.${key} must be string`);
    }
    return record;
}
function inputArray(value, id) {
...
function thinkingMap(value) {
    ...
    for (const [key, entry] of Object.entries(record)) {
        if (entry !== null && typeof entry !== "string")
            throw new Error(`models.json thinkingLevelMap.${key} is invalid`);
    }
    return record;
}
function costObject(value, partial) {
    ...
    if (record.tiers !== undefined) {
        if (!Array.isArray(record.tiers))
            throw new Error("models.json cost.tiers must be an array");
        for (const tier of record.tiers) {
            ...
            for (const key of ["input", "output", "cacheRead", "cacheWrite", "inputTokensAbove"]) {
                if (typeof entry[key] !== "number" || !Number.isFinite(entry[key]))
                    throw new Error(`models.json cost tier ${key} must be numeric`);
            }
        }
    }
    return record;
